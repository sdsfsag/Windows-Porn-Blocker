...
def main_menu(stdscr):
    ...
    menu_actions = {
        ...
        'Info': show_info,
        'Alles blockieren (Dauert am längsten) ~ 2,7 Minuten': block_all_sites,
        'Manuell blockieren': manual_block_menu,
        'Sex-Game-Blocker': game_block_menu, 
        'Beenden': exit_program,
        'PBlocker EN': english_version,
        ...
        if key == curses.KEY_UP:
            ...
            if selected_index < len(sites):
                ...
                for domain, ip_address in domains:
                    ...
                    loading_screen(stdscr, ip_address, site_name, percentage)
                stdscr.clear()
                stdscr.addstr(max_y // 2, (max_x - len(f"{site_name} wurde blockiert!")) // 2, f"{site_name} wurde blockiert!")
                stdscr.refresh()
                time.sleep(2)
        elif key == 27:  # ESC
            break
