...
def block_ip(ip_address, domain):
    try:
        ...
        print(f"An error occured while blocking the IP for {domain}: {e}")
...
def block_all_sites(stdscr):
    ...
    stdscr.clear()
    message = "All sites were blocked succesfully"
...
def unblock_last_action():
    ...
    try:
        ...
        print(f"An error occured while unblocking the IP of {domain}: {e}")

def loading_screen(stdscr, ip_address, site_name, percentage):
    stdscr.clear()
    stdscr.addstr(0, 0, f"Please wait... ({percentage}% finished)")
    stdscr.addstr(1, 0, f"Blocking {site_name} - IP: {ip_address}")
    stdscr.refresh()
    time.sleep(0.2)
...
def parent_version_function(stdscr):
    # Placeholder-Funktion für 'Parent Version'
    webbrowser.open("https://github.com/sdsfsag/Windows-Porn-Blocker/blob/main/Porn%20Blocker%20PV%20Beta.exe")

def german_version(stdscr):
    webbrowser.open("https://github.com/sdsfsag/Windows-Porn-Blocker/blob/main/Porn%20Blocker%20DE%20Beta.exe")
...
def show_info(stdscr):
    stdscr.clear()
    info_text = """Difference between 'Porn Blocker' and 'Porn Blocker PV'. The difference is that the PV (Parent Version) contains a few more features, such as running the program on your child's computer without them noticing.

            ===BLOCKING===

            'Block everything' - Blocks everything.

            'Strict block' - blocks everything + sites that aren't harmful but can be used for pornographic purposes.

            'Manual block' - may be misunderstood, but here you can manually block all available sites.

            'Sex-Game Blocker' - the same as 'Manual block', but with sex games

            'Strict Mode' - blocks sites/apps that have nothing to do with pornography but could be used for it."

            ===SUPPORT===

            'Support' - opens a page where you can send a message to the developer (complaints, feedback, etc.)

            'Info' - shows information about the program.


            Under the '|===OTHER=VERSIONS===|' category, you can find other versions of 'Porn Blocker'
        """
    max_y, max_x = stdscr.getmaxyx()
    ...
    for i, line in enumerate(lines):
        ...
        if stripped_line.startswith('===') and stripped_line.endswith('==='):
            ...
            if stripped_line == "'Info' - shows information about the program.":
    ...
    display_height = max_y - start_y - 2  # Reserviere 2 Zeilen für Anweisungen                                 #https://github.com/sdsfsag/Windows-Porn-Blocker/blob/main/Porn%20Blocker%20EN%20Beta.exe
    ...
    while True:
        ...
        instructions = "(Use arrow keys to scroll, ESC to return)"
...
def main_menu(stdscr):
    ...
    columns = [
        ...
        },
        {
            'header': '|===BLOCKING===|',
            'items': [
                'Block everything (takes the longest) ~ 2.4 minutes',
                'Manual block',
                'Sex-Game Blocker',
                'Exit'
            ]
    ...
    menu_actions = {
        ...
        'Info': show_info,
        'Block everything (takes the longest) ~ 2.7 minutes': block_all_sites,
        'Manual block': manual_block_menu,
        'Sex-Game Blocker': game_block_menu, 
        'Exit': exit_program,
        'Normal PBlocker DE': german_version,
        'PBlocker PV DE': parent_version_function
    }
    ...
    while True:
        ...
        for col_idx, column in enumerate(columns):
            ...
            stdscr.addstr(y_start, x_start + (col_width - len(header)) // 2,
                          header, curses.A_BOLD | curses.A_UNDERLINE | curses.color_pair(3))
            ...
            for item_idx, item in enumerate(column['items']):
                ...
                x = x_start + (col_width - len(item)) // 2  # Zentriert in der Spalte
...
def manual_block_menu(stdscr):
    ...
    while True:
        ...
        stdscr.addstr(0, 0, "Press ESC to return.", curses.color_pair(1))
        ...
        if key == curses.KEY_UP:
            ...
            if selected_index < len(sites):
                ...
                for domain, ip_address in domains:
                    ...
                    loading_screen(stdscr, ip_address, site_name, percentage)
                stdscr.clear()
                stdscr.addstr(max_y // 2, (max_x - len(f"{site_name} was blocked!")) // 2, f"{site_name} was blocked!")
                stdscr.refresh()
                time.sleep(2)
        elif key == 27:  # ESC
            break
...
def game_block_menu(stdscr):
    ...
    while True:
        ...
        draw_version(stdscr)  # Display version
        ...
        if key == curses.KEY_UP:
            ...
            if current_index > 0:
                ...
                if current_page > 0:
                    current_page -= 1
                    current_index = min(len(game_sites[(current_page + 1) * page_size - page_size:]) - 1, page_size - 1)
        elif key == curses.KEY_RIGHT:
            if current_index < len(page_sites) - 1:
                current_index += 1
            else:
